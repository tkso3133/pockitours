	...
	for (var i=0; i<len; i++) {
		...
		if (!tourHasStarted) {
			...
			if (i == 0) {
				...
				$('#bottom').append('<div class="startButton"><a class="aStartButton" href="javascript:startTour();"><label id="lblStart">Start</label></a></div>');
			}
		}
			...
			if (i == 0) {
				$('#bottom').append('<div class="startOverButton"><a class="aStartOverButton" href="javascript:startOverTour();">Start again</a></div>');
			}
		} else {
			if (location.location_completed == 'true') {
				...
				if (firstNonCompleted == true) {
					$('#bottom').append('<div class="resumeButton"><a class="aResumeButton" href="javascript:resumeTour();">Resume</a></div>');
...
function SetFontSizesAndDimensionsBasedOnTotalHeight() {
	...
	createCSSClass('.liSelectDifferentTour a, .liIntro a', 'font-size: ' + (totalHeight * .045) + 'px;');
	...
	createCSSClass('.blueTourInformationBar label', 'font-size: ' + (totalHeight * .03) + 'px; float: left; margin: 0px 0 0 10px;');
	...
	createCSSClass('.startButton', 'height: ' + (totalHeight * .08) + 'px; margin: 0 auto;');
	createCSSClass('.startButton a', 'width: 100%; font-size: ' + (totalHeight * .05) + 'px;');
}
...
function showAlert() {
	navigator.notification.alert('Welcome to Pockitours Charleston!  To get started browse through the available' +
